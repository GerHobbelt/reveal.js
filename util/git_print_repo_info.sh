...
-b      : Print the currently checked out BRANCH name (symbolic) or commit hash.
          ...
          When you do NOT want the utility to fall back to printing the commit hash
          when running in 'detached HEAD' mode, but instead want to see nothing and get
          and ERROR CODE in return (error code: 4), then run this command with option '-B'
          instead.
